...
def get_data( wa_type, folder_name):
    ...
    commands = [
        ...
        f"adb shell 'su root mkdir /storage/self/primary/{folder_name}'",

        # 2. Copy data WhatsApp (Database) ke directory primary HP dengan hak akses root
        f"adb shell 'su root cp -r /data/data/com.whatsapp/ /storage/self/primary/{folder_name}/'" if wa_type == 'wa_regular' 
            else f"adb shell 'su root cp -r /data/data/com.whatsapp.w4b/ /storage/self/primary/{folder_name}/'",

        # 3. Copy data WhatsApp (Media) ke directory primary HP dengan hak akses root
        f"adb shell 'su root cp -r /storage/self/primary/WhatsApp/ /storage/self/primary/{folder_name}/'" if wa_type == 'wa_regular' 
            else f"adb shell 'su root cp -r /storage/self/primary/WhatsApp\\ Business/ /storage/self/primary/{folder_name}/'",

        # 4. Pull data ke Desktop (tidak perlu `su root`)
        f"adb pull /storage/self/primary/{folder_name}/ ~/Desktop/",

        # 5. Copy database msgstore.db dan wa.db ke directory project web forensic (tidak perlu `adb shell`)
        ...
        f"adb shell 'su root rm -rf /storage/self/primary/{folder_name}'"
    ]
